...
const IMAGE_MAP_URL =
  process.env.IMAGE_MAP_URL ||
  'https://i.skinscore.app/cs2-data/images/images.json';

const IMAGE_MAP_FILE =
  process.env.IMAGE_MAP_FILE || 'game_files/images.json';
...
function buildWeaponInvKey(weaponToken, paintkitToken) {
  if (!weaponToken) return null;
  if (paintkitToken && paintkitToken !== 'default') {
    return `econ/default_generated/${weaponToken}_${paintkitToken}_light`;
  }
  return `econ/weapons/base_weapons/${weaponToken}`;
}

function buildStickerInvKey(materialPath) {
  if (!materialPath) return null;
  return `econ/stickers/${String(materialPath).trim()}`;
}

function buildAgentInvKey(characterToken) {
  if (!characterToken) return null;
  return `econ/characters/${String(characterToken).trim()}`;
}
...
class GameData {
  ...
  loadFiles() {
    ...
    if (fs.existsSync(IMAGE_MAP_FILE)) {
      try {
        this.image_map = JSON.parse(fs.readFileSync(IMAGE_MAP_FILE, 'utf8'));
      } catch {
        this.image_map = {};
      }
    } else {
  ...
  update() {
    ...
    utils.downloadFile(IMAGE_MAP_URL, (data) => {
      if (data) {
        try {
          const obj = JSON.parse(data);
          this.image_map = obj;
          fs.writeFileSync(IMAGE_MAP_FILE, data, 'utf8');
          winston.debug('Fetched images.json (Skinscore hosted)');
        } catch {
          winston.error('images.json is not valid JSON');
        }
      } else winston.error('Failed to fetch images.json');
    });
  }

  resolveImageUrl(invKey) {
    if (!this.image_map) this.image_map = {};
    const k = normalizeInvKey(invKey);
    if (!k) return null;

    return (
      this.image_map[k] ||
      this.image_map[`${k}_png`] ||
      this.image_map[`${k}_light`] ||
      this.image_map[`${k}_light_png`]
    ) || null;
  }
  ...
  addAdditionalItemProperties(iteminfo) {
    ...
    for (const sticker of iteminfo.stickers || []) {
      ...
      if (sticker.material) {
        const stickerUrl = this.resolveImageUrl(buildStickerInvKey(sticker.material));
        if (stickerUrl) {
    ...
    for (const keychain of iteminfo.keychains || []) {
      ...
      const keychainUrl = this.resolveImageUrl(normalizeInvKey(invPath));
      if (keychainUrl) {
    ...
    if (looksLikeAgentToken || iteminfo.weapon_type === 'Agent') {
      const agentUrl = this.resolveImageUrl(buildAgentInvKey(weaponToken));
      if (agentUrl) {
      ...
      const stickerUrl = this.resolveImageUrl(buildStickerInvKey(mat));
      if (stickerUrl) {
      ...
      const url = this.resolveImageUrl(buildWeaponInvKey(weaponToken, paintkitToken));
      if (url) {
