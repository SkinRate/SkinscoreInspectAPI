 ...
 * - weaponToken: internal weapon id (e.g. "weapon_knife_stiletto", "weapon_deagle")
 * - paintkitToken: internal paintkit id (e.g. "sp_dapple", "hy_ddpat_urb"); if falsy -> base weapon
 * Returns a PNG URL under /econ.
 */
 ...
 * Example material: "antwerp2022/sig_rox_holo" -> .../stickers/antwerp2022/sig_rox_holo_png.png
 */
 ...
 * characterToken is usually items_game.items[defindex].name (e.g. "customplayer_tm_professional_varh")
 */
...
class GameData {
  ...
  /*
      Loads items_game, csgo_english, and schema from disk
  */
  loadFiles() {
  ...
  /*
      Updates and saves the most recent versions of csgo_english, items_game, and schema from the SteamDB Github
  */
  update() {
  ...
  /*
      Given returned iteminfo, finds the item's min/max float, name, weapon type, and image url using CS2 game data
  */
  addAdditionalItemProperties(iteminfo) {
    if (!this.items_game || !this.csgo_english) return;

    // Get sticker codename/name (+ image URL for each sticker)
    ...
    for (const sticker of iteminfo.stickers || []) {
      ...
      // Attach sticker image URL (ByMykel)
      if (sticker.material) {
    ...
    // Get keychain name (image mapping can be added later similarly)
    ...
    // Get the skin token suffix (e.g., "_sp_dapple") or empty for vanilla
    let skin_name = '';
    if (iteminfo.paintindex in this.items_game['paint_kits']) {
      skin_name = '_' + this.items_game['paint_kits'][iteminfo.paintindex]['name'];
      if (skin_name === '_default') skin_name = '';
    }

    // Get the weapon/character internal token (e.g., "weapon_knife_stiletto" OR "customplayer_tm_professional_varh")
    let weapon_name;
    if (iteminfo.defindex in this.items_game['items']) {
      weapon_name = this.items_game['items'][iteminfo.defindex]['name'];
    }

    // ----- IMAGE URL RESOLUTION (ByMykel only) -----
    const paintkit =
    ...
    // Detect Agents reliably via internal token
    ...
    } else if (iteminfo.weapon_type === 'Sticker' && Array.isArray(iteminfo.stickers) && iteminfo.stickers[0]) {
      // Primary image for sticker items: show the sticker itself
      const mat = iteminfo.stickers[0].material; // ex: "antwerp2022/sig_rox_holo"
      const stickerUrl = buildImageTrackerStickerUrl(mat);
      if (stickerUrl) {
        iteminfo.imageurl = stickerUrl;
        iteminfo.image_repo = IMAGE_TRACKER_REPO;
      }
    } else if (iteminfo.weapon_type === 'Charm' && Array.isArray(iteminfo.keychains) && iteminfo.keychains[0]) {
      // Future: add keychain mapping here if desired.
    } else {
      // Weapons (default or skinned)
    ...
    // ----- END IMAGE URL RESOLUTION -----

    // ✅ If it's an Agent, prefix *all* sticker display names with "Patch | "
    ...
    // Get the weapon_hud token to localize the weapon/agent display name
    let weapon_hud;
